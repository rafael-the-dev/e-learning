...
import type { ArchiveAcademicYearSchema } from "@/modules/academic-calendar/schemas/academic-year.schema";

export class ArchiveAcademicYearCommand extends BaseCommand<
...
> {
  async validate(): Promise<void> {
    ...
    if (year.status === "ARCHIVED") {
      throw new ValidationError("Dados inválidos", {
        academicYearId: ["Este ano letivo já está arquivado"],
      });
    }
  ...
  async authorize(): Promise<void> {
    ...
    if (!createAbility(perms).can(PERMISSIONS.ACADEMIC_YEARS_ARCHIVE)) {
      throw new AuthorizationError();
    }
  }

  async execute(): Promise<void> {
    await archiveAcademicYear(this.input.academicYearId, this.context.organizationId);

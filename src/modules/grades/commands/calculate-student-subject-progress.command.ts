...
import {
  ...
  type CalculateStudentSubjectProgressSchema,
} from "@/modules/grades/schemas/grade.schema";
import type { StudentSubjectProgress } from "@/modules/assessments/types";

export class CalculateStudentSubjectProgressCommand extends BaseCommand<
  CalculateStudentSubjectProgressSchema,
  StudentSubjectProgress
> {
  async validate(): Promise<void> {
    const result = calculateStudentSubjectProgressSchema.safeParse(this.input);
    if (!result.success) {
  ...
  async authorize(): Promise<void> {
    ...
    if (!createAbility(perms).can(PERMISSIONS.GRADES_CALCULATE)) {
  ...
  async execute(): Promise<StudentSubjectProgress> {
    ...
    const isTerminal =
      calculationResult.status === "PASSED" || calculationResult.status === "FAILED";
    ...
    await auditService.log(this.context, {
      ...
      action: "student_subject_progress.calculated",
      newValues: {
        studentId,
        levelSubjectId,

...
import { CalendarClock, ClipboardCheck, Wallet, GraduationCap, CalendarDays } from "lucide-react";
import type { UpcomingDeadline, UpcomingDeadlineType } from "@/modules/dashboard/types";

const TYPE_ICONS: Record<UpcomingDeadlineType, React.ReactNode> = {
  ASSESSMENT: <ClipboardCheck className="size-4 text-indigo-600" />,
  INSTALLMENT: <Wallet className="size-4 text-amber-600" />,
...
export function DashboardUpcomingDeadlines({ items }: Props) {
  if (items.length === 0) {
    return (
      <EmptyState
        icon={<CalendarClock className="size-8" />}
        title="Sem prazos próximos"
        description="Não há prazos nos próximos 30 dias."
        className="border-0"
      />
    );
  }

...
export class DeactivateNotificationTemplateCommand extends BaseCommand<TemplateIdSchema, NotificationTemplate> {
  ...
  async execute(): Promise<NotificationTemplate> {
    const deactivated = await deactivateTemplate(this.context.organizationId, this.input.templateId);

    await auditService.log(this.context, {
      entity: "NotificationTemplate",
      entityId: deactivated.id,
      action: "notification_template.deactivated",
      newValues: { eventType: deactivated.eventType, channel: deactivated.channel },
    });

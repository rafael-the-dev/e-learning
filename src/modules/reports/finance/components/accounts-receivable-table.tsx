...
import type { AccountsReceivableRow } from "@/modules/reports/finance/types";
...
const columns: ColumnDef<AccountsReceivableRow>[] = [
  {
  ...
  },
  {
    ...
    header: "Pago",
    cell: ({ row }) => (
      <span className="font-mono text-sm text-emerald-600">{row.original.paidAmount.toLocaleString("pt-PT", { minimumFractionDigits: 2 })} MZN</span>
    ),
  },
  {
    ...
    header: "Saldo",
    cell: ({ row }) => (
      <span className="font-mono text-sm font-semibold text-red-600">{row.original.balanceAmount.toLocaleString("pt-PT", { minimumFractionDigits: 2 })} MZN</span>
    ),
  },
  {
    accessorKey: "status",
...
export function AccountsReceivableTable({ queryString }: Props) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["report-ar", queryString, pagination],
    queryFn: async () => {
      const params = new URLSearchParams(queryString);
      params.set("page", String(pagination.pageIndex + 1));
      params.set("pageSize", String(pagination.pageSize));
      const res = await fetch(`/api/reports/finance/accounts-receivable?${params}`);

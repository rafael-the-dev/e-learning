...
import { ImportInstructionsCard } from "@/modules/students/import/components/import-instructions-card";
import { UploadCard } from "@/modules/students/import/components/upload-card";
import { ValidationResults } from "@/modules/students/import/components/validation-results";
import { PreviewTable } from "@/modules/students/import/components/preview-table";
import { ImportSummary } from "@/modules/students/import/components/import-summary";
import type { ImportReport, ValidationResult } from "@/modules/students/import/types";
...
export function ImportWizard() {
  ...
  async function handleValidate(file: File) {
    ...
    try {
      ...
      const res = await fetch("/api/students/import/validate", { method: "POST", body: formData });
  ...
  async function handleExecute() {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(body.error ?? "Erro ao importar os alunos");
        return;
      }
      setImportReport(body);
      setStep("result");
      toast.success(`${body.importedCount} aluno(s) importado(s) com sucesso`);
      router.refresh();
    } catch {

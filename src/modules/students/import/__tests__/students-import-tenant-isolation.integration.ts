/**
 * Students Import — Tenant Isolation Integration Test Script
 * =============================================================
 ...
 *    including students.import, must already exist).
 *
 * Run
 * ---
 *   npx tsx src/modules/students/import/__tests__/students-import-tenant-isolation.integration.ts
 *
 * Scenarios
 * ---------
 *  1. Full validate -> execute pipeline for Org A creates a student scoped
 *     to Org A, with audit entries scoped to Org A.
 *  2. Org B's admin cannot execute Org A's import job; no student leaks
...
import { ExecuteStudentsImportCommand } from "@/modules/students/import/commands/execute-students-import.command";
...
const CSV_HEADER =
  "firstName,lastName,gender,birthDate,phone,email,documentType,documentNumber,address";
...
async function setup(db: PrismaClient): Promise<Fixture> {
  ...
  const emailDomain = `import-tenant-iso-${runId}.test`;
  ...
  const orgA = await db.organization.create({
    data: { name: `Import Tenant Iso A ${runId}`, slug: `import-tenant-iso-a-${runId}` },
  });
  const orgB = await db.organization.create({
    data: { name: `Import Tenant Iso B ${runId}`, slug: `import-tenant-iso-b-${runId}` },
  });

  const adminA = await db.user.create({
    data: { name: "Import Admin A", email: `admin-a@${emailDomain}`, isActive: true },
  });
  const adminB = await db.user.create({
    data: { name: "Import Admin B", email: `admin-b@${emailDomain}`, isActive: true },
  });
...
async function teardown(db: PrismaClient, f: Fixture): Promise<void> {
  ...
  await db.student.deleteMany({ where: { organizationId: { in: orgIds } } });
...
async function scenario1(f: Fixture): Promise<{ jobId: string; studentDocNumber: string }> {
  console.log(
    "\n── 1. Full validate -> execute pipeline creates a student scoped to Org A ──"
  );
  const docNumber = `TENANTISO1-${f.runId}`;
  const buffer = buildCsvBuffer({
    firstName: "Aluno",
    lastName: "TesteA",
    email: `aluno-a@${f.emailDomain}`,
    documentNumber: docNumber,
  });

  const validation = await new ValidateStudentsImportCommand(
    { fileName: "alunos.csv", fileSize: buffer.byteLength, buffer },
    f.ctxA
  ).run();
  assert(validation.validRows === 1, "row validates as VALID for Org A");

  const report = await new ExecuteStudentsImportCommand({ jobId: validation.jobId }, f.ctxA).run();
  assert(report.importedCount === 1, "1 student imported for Org A");

  const db = await getDb();
  const created = await db.student.findFirst({
    where: { organizationId: f.orgA.id, idNumber: docNumber },
  });
  assert(created !== null, "the created student is scoped to Org A's organizationId");
  ...
  return { jobId: validation.jobId, studentDocNumber: docNumber };
}

async function scenario2(f: Fixture): Promise<void> {
  console.log(
    "\n── 2. Org B cannot execute Org A's import job; no student leaks into Org B ──"
  );
  ...
  await assertRejects(
    new ExecuteStudentsImportCommand({ jobId: validation.jobId }, f.ctxB).run(),
    "Org B's admin cannot execute Org A's import job"
  );

  const db = await getDb();
  const leaked = await db.student.findFirst({
    where: { organizationId: f.orgB.id, idNumber: docNumber },
  });
  assert(leaked === null, "no student was created in Org B as a side effect");
...
async function scenario4(f: Fixture, db: PrismaClient): Promise<void> {
  ...
  // Pre-seed a real student in Org B with this documentNumber.

...
const { createMany, transaction } = vi.hoisted(() => ({
  ...
  transaction: vi.fn(async (fn: (tx: { student: { createMany: typeof createMany } }) => Promise<unknown>) =>
    fn({ student: { createMany } })
  ),
}));
...
import { ExecuteStudentsImportCommand } from "../execute-students-import.command";
import type { ImportRowResult } from "@/modules/students/import/types";
...
describe("ExecuteStudentsImportCommand — validation", () => {
  ...
  it("throws NotFoundError when the job does not exist (or belongs to another org)", async () => {
    ...
    const cmd = new ExecuteStudentsImportCommand({ jobId: "missing" }, CTX);
...
describe("ExecuteStudentsImportCommand — authorization", () => {
  beforeEach(() => vi.clearAllMocks());
...
describe("ExecuteStudentsImportCommand — execute", () => {
  beforeEach(() => vi.clearAllMocks());
  ...
  it("skips ERROR rows, imports VALID/WARNING rows via createMany, and marks the job COMPLETED", async () => {
    ...
    expect(createMany).toHaveBeenCalledWith({
      data: expect.arrayContaining([
        expect.objectContaining({ organizationId: "org-1", firstName: "Aluno1" }),
        expect.objectContaining({ organizationId: "org-1", firstName: "Aluno2" }),
      ]),
    });
  ...
  it("isolates a failing chunk: marks only that chunk FAILED and still imports the rest", async () => {
    ...
    (findImportJobById as Mock).mockResolvedValue(makeJob({ rowsData: rows, totalRows: 150 }));
    transaction
      .mockImplementationOnce(async (fn: (tx: { student: { createMany: typeof createMany } }) => Promise<unknown>) =>
        fn({ student: { createMany } })
      )

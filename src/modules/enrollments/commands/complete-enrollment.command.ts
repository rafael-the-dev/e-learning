...
export class CompleteEnrollmentCommand extends BaseCommand<CompleteEnrollmentSchema, Enrollment> {
  ...
  async validate(): Promise<void> {
    const result = completeEnrollmentSchema.safeParse(this.input);
    if (!result.success) {
    ...
    if (!allowed.includes("COMPLETED")) {
      throw new BusinessRuleError(
        `Não é possível concluir uma matrícula com estado "${existing.status}".`
      );
    }
  ...
  async authorize(): Promise<void> {
    ...
    if (!createAbility(perms).can(PERMISSIONS.ENROLLMENTS_COMPLETE)) {

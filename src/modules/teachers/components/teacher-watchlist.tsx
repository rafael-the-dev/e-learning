...
import type { TeacherWatchlistItem, WatchlistSeverity } from "@/modules/teachers/services/teacher-watchlist.service";

interface Props {
  items: TeacherWatchlistItem[];
}
...
export function TeacherWatchlist({ items }: Props) {
          ...
          >
            ...
            <div className="flex flex-col items-end gap-1 shrink-0">
              ...
              <span className="text-[10px] tabular-nums text-muted-foreground">
                {item.subjectCount} disc. · {item.activeClassGroupCount} turma(s)
              </span>
            </div>
          </div>
        );

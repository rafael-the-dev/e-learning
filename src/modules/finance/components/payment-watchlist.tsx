...
interface PaymentWatchlistProps {
  items: PaymentWatchlistItem[];
}

export function PaymentWatchlist({ items }: PaymentWatchlistProps) {
  if (items.length === 0) return null;

  return (
    <div className="divide-y">
      {/* Desktop / tablet header */}
      <div className="hidden md:grid grid-cols-[1fr_auto_1fr_auto_1fr_auto_auto] gap-4 px-5 py-2 text-xs font-medium text-muted-foreground">
        <span>Aluno</span>
        <span>N.º Pagamento</span>
        <span>Fatura</span>
      ...
      {items.map((item) => {
        ...
        return (
          <div key={item.paymentId} className="hover:bg-muted/30 transition-colors">
            {/* Mobile card layout */}
            <div className="md:hidden px-4 py-3 space-y-2">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground font-mono">
                    {item.paymentNumber}
                    {item.invoiceNumber && (
                      <span className="ml-2 text-muted-foreground">· {item.invoiceNumber}</span>
                    )}
                  </p>
                </div>
                <span
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-1.5 min-w-0">
                  ...
                  <span className="text-xs text-muted-foreground truncate">{item.issue}</span>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <Button asChild variant="ghost" size="sm" className="h-7 px-2 text-xs">
                    <Link href={`/payments/${item.paymentId}`}>
            ...
            {/* Desktop table row */}
            <div className="hidden md:grid grid-cols-[1fr_auto_1fr_auto_1fr_auto_auto] gap-4 px-5 py-3 items-center">
              {/* Student */}
              <div className="min-w-0">
                {item.studentId ? (
                  <Link
                ...
                <p className="text-xs text-muted-foreground">
                  {item.paymentDate.toLocaleDateString("pt-PT")}
                </p>
              </div>
              ...
              <div className="min-w-0">
                {item.invoiceId ? (
                  ...
                  <span className="text-sm text-muted-foreground italic">Sem fatura</span>
                )}
              </div>
